...
THIS_PIP=${P2P_6}
THIS_HIP=${HOST_6}
sudo docker run -d -p 2980:2980 -p 4601:4601 --net=host --name ${THIS_NAME} quay.io/coreos/etcd:${ETCD_VERSION} \
    --name ${THIS_NAME} \
    --initial-advertise-peer-urls http://${THIS_PIP} --listen-peer-urls http://${THIS_PIP} \
    --advertise-client-urls http://${THIS_HIP} --listen-client-urls http://${THIS_HIP} \
    --initial-cluster ${CLUSTER} \
...
THIS_PIP=${P2P_7}
THIS_HIP=${HOST_7}
sudo docker run -d -p 3080:3080 -p 4701:4701 --net=host --name ${THIS_NAME} quay.io/coreos/etcd:${ETCD_VERSION} \
    --name ${THIS_NAME} \
    --initial-advertise-peer-urls http://${THIS_PIP} --listen-peer-urls http://${THIS_PIP} \
    --advertise-client-urls http://${THIS_HIP} --listen-client-urls http://${THIS_HIP} \
    --initial-cluster ${CLUSTER} \
